...
### 2.8 `migrations/0011_dash_hardening.sql`

Post-P12 hardening closeout (doc 12 OI-SEC-8, OI-API-8, OI-P6-3). Three additions, all
new-object/additive (online-safe by construction):
...
Registry additions: both tables are Class T with the standard isolation policy and **no**
operator cross-tenant policy (`mfa_used_steps` joins `sessions`/`mfa_recovery_codes` in the
never-operator-readable set). Owners: `internal/dash/security` (mfa_used_steps),
`internal/dash/httpx` (dash_admin_idempotency). `alert_rules.anomaly_floor_credits` stays with
the alerts owner.

### 2.9 `migrations/0012_dash_provisioning_mfa.sql`

Pending-completion work (doc 15 Part 1): operator Tenant provisioning + first-admin invites
(SEC-3, ADR-0021), the per-Tenant MFA knob (SEC-5), bulk-job cancellation (OI-API-4), and the
key-level cost dimension (RF-3). All additive except the two widen-in-place ALTERs on the small
`bulk_jobs` and `cost_rollup_1d` config/rollup tables.
...
Registry additions: `tenant_invites` is Class T with the standard isolation policy and **no**
operator cross-tenant policy (invites are Tenant-private; the accept path is token-authenticated).
Owner: `internal/dash/provisioning`. The `tenants`/`bulk_jobs`/`cost_rollup_1d` ALTERs keep their
existing owners and policies.
