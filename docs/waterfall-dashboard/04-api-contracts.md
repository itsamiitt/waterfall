...
| GET | `/research/runs` | Research run lifecycle rows (async lane), newest first | TU+ (own Tenant) / O | `{ items: [{ run_id, subject_key, status, config_version, created_at, updated_at }] }`; status ∈ queued/running/done/failed; `?limit=` (cap 200) |
...
|----|------|--------|-------|
| OI-API-1 | `openapi-admin.yaml` authored incrementally during P0–P11 with `TestAdminOpenAPIParity` mirroring `docs/api/openapi.json`; this doc is authoritative until parity is green. | RESOLVED (closeout: openapi-admin.{json,yaml} + apispec parity test 145==145) | Senior Backend Engineer |
...
| OI-API-8 | **P0 admin idempotency ledger is in-process (Deviation D-P0-2).** The §1.3 contract (identical body replays with `Idempotency-Replayed: true`; different body → 409 `idempotency_key_reuse`) is honored exactly; only the backing store is in-memory in P0. A durable `(tenant_id, key)` ledger table replaces it in the phase that also backs approval-gated exactly-once execution. | RESOLVED (closeout: durable dash_admin_idempotency ledger, migration 0011) | Senior Backend Engineer |
