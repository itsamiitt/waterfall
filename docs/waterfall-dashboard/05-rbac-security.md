...
| SEC-2 | WORM anchor sink selection (S3 Object Lock vs equivalent) and anchor cadence — design-target in §8.3; finalize with deployment topology in doc 11 | OPEN | Solutions Architect |
...
| SEC-4 | Fingerprint pepper rotation: re-fingerprinting requires plaintext, so it can only occur at rotate/import; document the limitation and procedure in ADR-0017 | OPEN | Senior Backend Engineer |
