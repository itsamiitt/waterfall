...
| OI-UI-3 | Bundle-size CI check and token-contrast test are named P8 gates but their harness (size-limit script in `vitest`/CI) is not yet specified; define in P8 plan. | RESOLVED (closeout: npm run check:ci = tsc+vitest+allowlist+orphan+build+bundle) | Enterprise UX Architect |
