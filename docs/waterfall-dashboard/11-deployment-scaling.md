...
| DS-3 | WORM anchor: **DECIDED** — S3 Object Lock (compliance mode) reference, Azure Blob immutability / GCP Bucket Lock equivalents, nightly cadence (tail exposure ≤ 24h). Deployment-time ops job, not app code; activates at cloud deploy (see doc 05 SEC-2). | DECISION RECORDED (activate at deploy) | GTM Infrastructure Engineer |
| DS-5 | **Design-target stores (Redis / ClickHouse / Kafka-Redpanda / Temporal) are a deployment/ops dependency, not remaining code work.** They exist only behind the Go interfaces of doc 02 §7 with Postgres-backed implementations shipped and tested (rollups on Postgres partitioned tables, pub/sub via the SSE poller + LISTEN/NOTIFY, the transactional-outbox queue, leader election via advisory locks). Swapping in a real backend is a per-store adapter + its provisioning, triggered by a measured PG-comfort breach (rollup volume → ClickHouse; SSE fan-out scale → Redis; multi-region transport → Redpanda; Action-volume spike QS-TMP-1 → Temporal). None is required for the single-cell reference deployment this build targets. | DECISION RECORDED (deploy-time, gated on measured need) | Solutions Architect |
