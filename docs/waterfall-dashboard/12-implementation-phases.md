...
> **Status:** ACCEPTED (P12 closure 2026-07-06; self-verified; user plan approval 2026-07-02) · **Owner:** Solutions Architect · **Last updated:** 2026-07-06 · **Gated by:** /architecture-review, /security-audit
...
| Realtime no-polling — SSE fan-out + server-side aggregation; DB load independent of client count | Principle | PASS | ADR-0019 / doc 04 §3.1 mandate ONE multiplexed EventSource per tab (GET /streams?topics=csv) with a binding QoS split (ticks coalesce; changed/fired/resolved/progress never dropped), Last-Event-ID replay against a 256-event per-topic ring with reset-on-overflow, and a scripted P11 grep forbidding component polling except the single central 15s SSE-disconnect refetch. The leader-elected aggregator makes DB reads O(instances) not O(clients), and fan-out saturation is self-monitored (dash_sse_clients / dash_sse_dropped_total) with a concrete widen-tick degradation lever (2s→10s). **P12 measured (2026-07-06):** `TestSSESoakLite` 200 clients / 20s → p50 2.01ms, **p99 12.27ms** (≤2s target), 14,200 ticks, zero dropped `*.changed` — doc 13 §6 L2; full 10-min / 500-client soak deferred to staging (OI-P12-1). |
...
> **P12 closure 2026-07-06:** 47 Go packages (`go build ./... && go vet ./...` clean) + 192 web
> unit tests (22 vitest files) green; `scripts/run-rls-test.sh` release-blocker suite green on
> PostgreSQL 17.10 (RLS zero-rows on all dashboard tables + G2 replay + approval/CSRF/idempotency
> negatives); TypeScript `tsc --noEmit`, no-orphan-UI, and the 111.2 KB-gz production bundle all
> green. **Live boot smoke passed** — dashboardd booted against an ephemeral PG17 with bootstrap
> (10 migrations + role provisioning), served the SPA + `/healthz` `/readyz` `/metrics`, rejected
> the unauthenticated admin route (401), completed a pbkdf2 login (operator → mfa_required,
> tenant_user → ok+csrf), and served six authenticated operator reads (audit-verify, queues,
> dead-letters, overview, workers, audit-log) 200. Single-instance measurements recorded in doc 13
> §6 (L1 24.7M sel/s, L2 SSE p99 12.27ms @ 200 clients, L3 1k-key import, L4 100k-event fold
> byte-identical). Residual full-scale / multi-instance / chaos / live-Playwright items are deferred
> to a staging load-lab and listed below as **OI-P12-1..3**.
...
| OI-IP-2 | Approval gating of P1 destructive actions (provider delete/archive, key bulk delete) is audited-inline until the approvals engine lands in P4, then wired through `approvals.Gate` in the P4 commit | RESOLVED (P4): `cmd/dashboardd` wires `apprSvc` (a `*approvals.Service`) as the optional `Gate` into `providers.Deps`, `keys.Deps`, `routing.Deps`, `workflows.Deps` (nil ⇒ `approvals.NopGate`, so prior behavior/tests are unchanged). Each gated handler pins the resolved payload and calls `Gate.Check(ctx, action_kind, payload)`: `err`→500, `!proceed`→202 `{approval_request_id}`, `proceed`→inline. Action kinds per handler: providers `DELETE`→`provider_delete`, providers `POST /archive`→`provider_archive`, keys `POST /bulk (op=delete)`→`key_bulk_delete` (payload = fully-resolved `{"ids":[…]}` via `keys.Service.ResolveKeyIDs`), routing publish+rollback→`routing_publish`, workflows publish+rollback→`workflow_publish` (gate applied in `configver`'s shared publish/rollback handlers via a structural `configver.Gate` seam so configver never imports approvals). On quorum the pinned payload runs EXACTLY ONCE through a registered `Executor` that drives the real service method. Proven E2E by `internal/dash/e2e.TestDashApprovalGateWiring`. | Solutions Architect |
...
| OI-KEYS-1 | P1 bulk-op + key-import execution is in-process (goroutine tracked by `key_import_batches`); it migrates to the durable `bulk_jobs` lease/janitor model in P5. `op=delete` executes inline-audited (approval-gated in P4 per OI-IP-2). | RESOLVED (Wave-1: keys bulk-op/import execute on the durable bulk_jobs lease model) | Senior Backend Engineer |
...
| OI-KEYS-3 | `GET /keys/{id}/usage` returns live `provider_keys` counters + an empty time-series in P1; the `key_usage_1m/1h/1d` rollups that populate the series arrive with P4 telemetry. | RESOLVED (Wave-1: usage feed via OI-P4-1 recorder; key_usage rollups populate live) | Senior Backend Engineer |
| OI-KEYS-4 | Key-pool strategy/membership writes persist + audit in P1; the doc 07 §10 `key_pool` config-epoch bump (`configver.BumpEpoch`) that invalidates in-memory `PoolState` is deferred to P3 (config versioning) — until then rotation reads the row directly. In P2 `rotation.Engine.Invalidate(selector)` is the documented hook the P3 epoch watcher will call; P2 stands in with rebuild-on-demand + a periodic refresh loop. | RESOLVED (P3): `configver.Service` owns the `BumpEpoch(ctx, tenantID, kind, scopeKey)` API; every publish bumps `config_epochs` in-tx and fires an `OnBump(tenantID, kind, scopeKey)` callback. `cmd/dashboardd` wires that callback so `kind ∈ {key_pool, provider_catalog}` calls `rotEngine.Invalidate(scopeKey)` — closing the P3 epoch-watcher hook. | Senior Backend Engineer |
...
| OI-SEC-9 | Step-up (`X-MFA-Code`) on keys create/import/rotate/bulk verifies TOTP against the caller's sealed seed (`cmd/dashboardd.totpStepUp`); accepting a one-time recovery code on the step-up path (as login-verify does) is deferred. | RESOLVED (Wave-1 + Wave-2/doc 15 §T5e: `security.Users.VerifyStepUp` accepts a fresh single-use TOTP OR a one-time recovery code; `cmd/dashboardd.totpStepUp` retargeted to it) | Senior Backend Engineer |
...
| D-2 | The `budgets` table CREATE + its Class-T RLS moved from migration 0008 into 0006 (doc 03 §2.3) because P3's routing/Waterfall validator (VR-7) cross-checks `max_cost_credits` against a real Tenant budget row against live schema. The DDL is copied verbatim from doc 03's 0008 block; 0008 leaves a "moved to 0006 per D-2" note. `internal/pgmigrate/migrate_integration_test.go`'s two teardown drop-lists gained the 0006 tables (config_versions, config_active, config_epochs, workflow_index, budgets) so the clean-slate re-apply stays green. | DECISION RECORDED (P3) | Senior Backend Engineer |
| OI-P4-1 | The live rotation→`usage_events` hot-path feed is DEFERRED. `cmd/dashboardd` constructs the full telemetry backbone (`telemetry.BufferedRecorder` Sink over `telemetry.NewRecorder`, plus the leader-elected `telemetry.Loops`: aggregator fold + partition maintainer + key-budget reconcile, started at boot with `defer Stop`), but no producer yet feeds the recorder from `rotation`'s `Lease.Done`. Reason: the `Lease.Done`/`recordOutcome` path carries only `keyState` (key id) + `provider.Outcome` (ok/class/latency); it has neither the `tenant_id` (REQUIRED for the Class-T `usage_events` insert, G1) nor the `provider_id`/`workflow_key`/`country` needed to build a correct `telemetry.UsageEvent`, so adding an `OnCall` sink to `Done` would be invasive and could not populate the mandatory dimensions. The telemetry machinery is fully exercised via synthetic events (`TestTelemetryFoldRefoldIdentical`, `…ReconcileKeyBudgets`, `…PartitionMaintainer`, `…LeaderElection`). Wiring the live feed needs the egress path to thread tenant/provider/workflow attribution into the lease Outcome — a later slice. Per instruction, this does NOT block the P4 gate. | RESOLVED (Wave-1 + Wave-2/doc 15 §T5c: OI-P4-1b closed — `/v1/enrichments` carries optional `workflow_key`/`country` (attribution-only, never in the G2 key), `engine.Run` tags the lease ctx via `engine.WithAttribution` → `provider` ctx keys, and rotation's `captureUsage` emits fully-attributed usage rows) | Senior Backend Engineer |
| OI-QW-7 | P5 queue_stats fold sampling deviates from doc 06 §6's single BYPASSRLS aggregate on the relay-role connection (the §6 approach was OI-QW-2, still awaiting /security-audit). `telemetry.Aggregator.QueueStatsFold` instead iterates the operator-readable customer-Tenant registry and counts each Tenant's `job_outbox` rows under that Tenant's own RLS transaction (identical to the usage-fold per-Tenant pass), then sums — functionally identical counts with strictly LESS privilege (no BYPASSRLS reader exists), and exercisable as a non-superuser (`TestQueueStatsFoldAndList`). It is a SEPARATE Aggregator method (never called by `Fold`/`Refold`), so the P4 fold-idempotency proof is untouched; the fold writes the whole per-(queue,bucket) row with last-sample-wins REPLACE semantics (idempotent re-sampling). | DECISION RECORDED (P5) | Senior Backend Engineer |
| OI-QW-8 | pgstore backend maps the entire single pgoutbox outbox to ONE logical queue name (`job_outbox` carries no queue column): the fold + queue list attribute the aggregate vector to the configured `queue_defs` row (e.g. `enrich-default`). Multi-queue topology arrives with the target engines behind the unchanged `QueueBackend` seam (doc 06 §1.3). | DECISION RECORDED (P5) | Senior Backend Engineer |
| OI-QW-9 | Queue list (`GET /queues`) serves the newest `queue_stats_1m` bucket (the fold's durable output) rather than doc 06 §6's leader in-memory copy / `self_monitor` follower snapshot — still never a per-request `COUNT(*)` over `job_outbox`, so the ≤ one-sample-old invariant holds. The in-memory snapshot, the `self_monitor` `queue_stats_sample` row, and the coalesced `queue.stats.tick` SSE are deferred to P7 realtime. | RESOLVED (P7: self_monitor snapshot + SSE tick) | Senior Backend Engineer |
| OI-KEYS-1b | P5 lands the durable `bulk_jobs` lease/janitor model: `queues` (replay, kind=`replay`) and `workers` (rolling-restart, kind=`rolling_restart`) write it and expose a kind-agnostic `GET /bulk-jobs/{id}` durable reader via `queues.BulkJobsRoute`. The orchestrator MUST mount exactly one owner of `GET /bulk-jobs/{id}` — `queues.BulkJobsRoute` (durable) — and retire the keys package's P1 in-process `GET /bulk-jobs/{id}` registration to avoid a net/http duplicate-pattern panic; migrating the keys bulk-op executor itself onto `bulk_jobs` is the remaining OI-KEYS-1 tail. The rolling-restart executor + janitor loop are provided but wired by the orchestrator (worker-lost detector loop + bulk janitor). NOTE (P5 wiring): `cmd/dashboardd` now mounts `queues.BulkJobsRoute` as the single `GET /bulk-jobs/{id}` owner and the keys package's in-process registration is removed (its registry stays behind `keys.Service.BulkStatus` for the P1 bulk-op executor); the worker-lost detector loop is started under the platform system Principal. The standalone bulk-job janitor loop is NOT yet exported by `queues`/`workers` (expired-lease re-queue currently relies on the executors' claim-on-`queued` path), so it remains open with the keys bulk-op migration. | RESOLVED (Wave-1 + Wave-2/doc 15 §T5b: OI-KEYS-1c closed — the janitor RE-QUEUES resumable kinds (key_import/key_bulk/replay) instead of failing them; the `keys`/`queues` BulkJobRunners (started in dashboardd) claim re-queued rows FOR UPDATE SKIP LOCKED under a fresh lease and resume past the durable cursor. In-file duplicate rows still error while resume catch-up rows skip idempotently (`insertedThisAttempt` discriminator); attempts capped; a survivor lacking the in-memory staged payload parks the job `failed` — key material is never persisted) | Senior Backend Engineer |
| OI-P5-1 | enrichd heartbeat wiring DEFERRED pending machine-auth config. `internal/heartbeat` (the enrichd-side client) is complete and proven against the real registry store (`TestWorkersDrainConverges`), but it abstracts the wire behind its `Transport` interface — no HTTP transport ships yet because `POST /v1/admin/workers/{id}/heartbeat` is authenticated + RBAC-gated (`rbac.WorkersActions`) behind the dashboard's FeatureChain, and enrichd has no dashboard credential story (the dashboardd machine-JWT verifier exists via `JWT_HS256_SECRET`, but no issuer/secret provisioning reaches enrichd). Wiring the beat loop into `cmd/enrichd` lands once a machine principal (operator-scoped JWT) is provisioned for workers. | RESOLVED (Wave-1 + Wave-2/doc 15 §T5a: OI-P5-2 closed — `api.Server.ShouldClaim` is wired to the heartbeat client, so a dashboard-set draining/stopped desired_state makes the worker refuse NEW submissions + dead-letter redrives with 503 `draining` + Retry-After while in-flight jobs finish; nil hook = unchanged behavior) | Senior Backend Engineer |
| OI-P4-2 | Approval-executor / audit-chain deadlock resolved. The gated Executors invoke the REAL service methods (`providers.Service.Delete`/`Archive`, `keys.Service.BulkOp`, `configver.Service.Publish`/`Rollback`), each of which appends to the caller-tenant hash-chained audit log on its own transaction — a `FOR UPDATE` on that tenant's `audit_chain_heads` row. The approvals decision runs the Executor INSIDE its own decision transaction; previously it audited the decision (also `FOR UPDATE audit_chain_heads`) BEFORE calling the Executor, so the decision tx held the chain-head lock while (in Go) awaiting the Executor, and the Executor's service tx blocked (in Postgres) on that same lock — a cross-connection cycle Postgres cannot detect (the decision tx is not itself lock-waiting), i.e. a hang. FIX (minimal, backward-compatible, in `approvals.Service.decide`): on the quorum path the Executor now runs BEFORE this connection takes the `audit_chain_heads` lock — `executeLocked` (the `approval_execute` audit + status write) and the winning-decision audit acquire that lock only AFTER the Executor returns and its service tx has committed + released it. Non-quorum approve and reject paths are unchanged (they audit as before). All approvals unit + integration tests stay green (`TestApprovalsExactlyOnce` verifies exactly-once + a clean chain under a 10-way race; audit-row ordering within the chain is not asserted and the chain re-verifies clean). Cross-connection execution atomicity is unchanged (an Executor error still parks the request `failed`, doc 05 §9.2). Proven by `TestDashApprovalGateWiring`. | RESOLVED (P4) | Senior Backend Engineer |
| OI-P6-1 | `cost.anomaly` is a 17th alert-rule metric beyond doc 10 §4's enumerated 16 (P6 deliverable requires the anomaly rule). Doc 10 §4 and the `GET /meta/enums` UI enum must absorb it in lockstep (P7/P11). | RESOLVED (Wave-1: cost.anomaly in doc 10 §4 + /meta/enums, parity-tested) | Senior Backend Engineer |
| OI-P6-2 | `system.sse_clients` / `system.aggregator_lag_s` alert metrics reference the `self_monitor` table which lands in P7; until then their evaluator branch is inert (query errors logged-and-skipped, never crashing the loop). | RESOLVED (P7: self_monitor exists; SSE/aggregator self-metrics live) | Senior Backend Engineer |
| OI-P6-3 | `cost.anomaly`'s absolute floor is a package constant (1000 credits) because `alert_rules` carries a single `threshold` (the percent); a per-rule absolute floor needs a rule-scope/schema extension. | RESOLVED (Wave-1: alert_rules.anomaly_floor_credits, migration 0011) | Senior Backend Engineer |
| OI-P6-4 | Alert episode/suppression audit is best-effort via `audit.AppendConn` on the transition tx (nil-safe); wired in dashboardd, nil in the gate integration tests. | DECISION RECORDED | Senior Backend Engineer |
| OI-P12-1 | Full-scale / multi-instance **load + chaos** runs deferred to a staging load-lab. Covered by in-proc P-gate equivalents (measured, doc 13 §6) but the standalone harnesses are to be built: doc 13 §6 L2 (500-client / 10-min SSE soak + reconnect storm, `scripts/load/sse_soak.go`), L3 (50k-row import, `TestImportLoad50k`), L4 (1M-event fold, `TestFold1M`), L5 (admin API p95 @ 200 rps, `scripts/load/api_load.go`); doc 13 §7 chaos PG-restart-reconnection + poison-import-row drills; and the RB-14 restore RPO/RTO (≤5m/≤1h) drill. The aggregator-leader failover, publish-crash consistency, and lease-batch (≤64) invariants ARE covered by existing tests (`TestOverviewAggregatorFailover`, `TestTelemetryLeaderElection`, `TestConcurrentPublishConflict`, `TestRotationLeaseNoOverLease`). | PARTIAL (Wave-2: 50k import + 1M fold measured single-instance dev, doc 13 §6; multi-instance/10-min/500-client soak deferred to staging) | Senior Backend Engineer |
| OI-P12-2 | **Playwright-against-live**: the doc 13 §5 E2E flows (login→overview→1k-key import→routing publish+rollback→DLQ redrive→alert fire/ack→RBAC deny-matrix) are authored and run against a seeded harness; the full run against a live booted dashboardd on a staging cluster is deferred. The P12 live-boot smoke proved the same server wiring end-to-end (login + operator reads) at the API layer. | RESOLVED (Wave-2: live Playwright login→MFA→overview passes; caught+fixed the SPA history-fallback bug) | Enterprise UX Architect |
| OI-P12-3 | **Publish-crash fault-point** (doc 13 §7 / OI-TS-5): the env-gated kill-mid-commit test hook for the dashboardd-kill-during-publish drill is not yet built (must never exist in production builds). Consistency is currently proven by `TestConcurrentPublishConflict` (exactly-one-winner + 409 `version_conflict`, config_active never non-validated); the fault-injection variant is deferred to staging. | RESOLVED (Wave-2: configver test-only publish-crash fault hook + PG-restart + poison-row chaos tests; invariants hold) | Senior Backend Engineer |
